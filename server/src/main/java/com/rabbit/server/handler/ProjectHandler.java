...
public class ProjectHandler {
        ...
        return exchange -> {
            ...
            try {
                ...
                if (segments.length >= 3 && !segments[2].isEmpty()) {
                    ...
                    Optional<ProjectDto> project = service.getProjectById(projectId);
    ...
    public HttpHandler create() {
        return exchange -> {
            ...
            try {
                ...
                service.createProject(userId, dto);
                send(exchange, 201, "{\"message\":\"Project created\"}");
            } catch (Exception e) {
                send(exchange, 500, "{\"error\":\"Internal server error\"}");
            }
        };
    ...
    public HttpHandler update() {
        return exchange -> {
            ...
            try {
                ...
                ProjectDto dto = mapper.readValue(exchange.getRequestBody(), ProjectDto.class);
                boolean updated = service.updateProject(projectId, userId, dto);
                send(exchange, updated ? 200 : 404,
                        updated ? "{\"message\":\"Updated\"}" : "{\"error\":\"Project not found\"}");
    ...
    public HttpHandler delete() {
        return exchange -> {
            ...
            try {
                int projectId = extractId(exchange.getRequestURI().getPath(), 2);
                boolean deleted = service.deleteProject(projectId, userId);
                send(exchange, deleted ? 200 : 404,
                        deleted ? "{\"message\":\"Deleted\"}" : "{\"error\":\"Project not found\"}");
            } catch (SecurityException e) {
                send(exchange, 403, "{\"error\":\"" + e.getMessage() + "\"}");
            } catch (Exception e) {
                send(exchange, 500, "{\"error\":\"Internal server error\"}");
